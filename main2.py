...
class train_model:
    ...
    def train(self):
        ...
        self.rfc.fit(mainarray, train_y)

    def test(self, test_data):
        try:
            test_predict = list(map(int, test_data))
            y_pred = self.rfc.predict([test_predict])
            return y_pred
        except:
            print("All Factors For Finding Personality Not Entered!")

    def predict_accuracy(self, training_dataset):
        """Predict the accuracy of the model using the validation dataset."""
        ...
        temp = df[7]
        validation_y = temp.values

        y_pred = self.rfc.predict(mainarray)
        accuracy = np.mean(y_pred == validation_y)
...
if __name__ == "__main__":
    ...
    model.predict_accuracy(training_dataset)
